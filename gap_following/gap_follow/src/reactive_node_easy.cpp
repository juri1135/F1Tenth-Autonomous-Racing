...
public:
    ReactiveFollowGap() : Node("reactive_node")
    {
        ...
        this->declare_parameter("emergency_dist",0.5); // 정면거리가 이것보다 가까우면 중앙으론 가지 마시오
        this->declare_parameter("safe_dist", 0.8); // 이 거리 이상일 때만 gap으로 인정 
        this->declare_parameter("disparityT",0.2);  // gap 찾을 때 최소한의 거리 차이

        this->declare_parameter("k_angle",3.0);     // 조향각에 따라 속도를 얼마나 강하게 줄일지 
        this->declare_parameter("front_ref", 1.2);   // 전방 거리 기준값. 키우면 속도 보수적, 가까운 장애물에 민감, 줄이면 가까워도 속도 유지 
        this->declare_parameter("max_speed",5.0); 
        this->declare_parameter("min_speed",2.0);
        

        this->declare_parameter("threshold_max",3.0);
        this->declare_parameter("threshold_mid",2.0);
        this->declare_parameter("threshold_min",1.2);

        this->declare_parameter("front_max", 2.5); // 정면 거리에 따라 threshold 조정
        this->declare_parameter("front_mid",1.2);
